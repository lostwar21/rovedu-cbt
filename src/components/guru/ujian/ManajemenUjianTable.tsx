...
import { Plus, Edit2, Trash2, X, Loader2, ClipboardList, Clock, BookOpen, Users, ShieldCheck, Shuffle, Eye, Key, Search, Filter } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center bg-muted/30 p-4 rounded-xl border border-border/50">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          >
            ...
            {/* Mengambil daftar mata pelajaran unik dari listUjian */}
            {Array.from(new Set(listUjian.map(u => u.mataPelajaran?.id))).map(id => {
              const mapel = listUjian.find(u => u.mataPelajaran?.id === id)?.mataPelajaran;
              return mapel ? <option key={id} value={id}>{mapel.nama}</option> : null;
            })}
          </select>
        </div>
      ...
      {filteredUjian.length === 0 ? (
        <div className="p-14 text-center glass rounded-[var(--radius-lg)] border border-dashed border-border">
          ...
          <p className="text-muted-foreground">
            {searchQuery || selectedMapel !== "all" 
              ? "Tidak ada ujian yang cocok dengan filter Anda." 
              : "Belum ada ujian yang dibuat."}
          </p>
          {!searchQuery && selectedMapel === "all" && <p className="text-sm text-muted-foreground mt-1">Klik tombol "Buat Ujian Baru" untuk memulai.</p>}
        </div>
      ) : (
